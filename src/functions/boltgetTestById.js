...
app.http('boltgetTestById', {
  ...
  handler: async (req) => {
    ...
    try {
      ...
      if (!test) {
        return { status: 404, jsonBody: { message: 'Test not found' } };
      }

      // 👇 Aggregation to get accurate question count
      const [aggResult] = await Test.aggregate([
        { $match: { _id: test._id } },
        {
          $project: {
      ...
      const questionCount = aggResult?.count || 0;
      ...
      if (authResult.user.role === 'student') {
        ...
        const safeTest = {
          ...
          createdBy: test.createdBy,
          allowedCenters: test.allowedCenters,
          questionCount, // ✅ use accurate count
          instructions: test.instructions,
          negativeMarking: test.negativeMarking,
          createdAt: test.createdAt
        };
      ...
      // For admin or non-student, include questionCount too
      const fullTest = {
        ...test.toObject(),
        questionCount
      };

      return { status: 200, jsonBody: { test: fullTest } };
    } catch (e) {
      console.error(e);
      return { status: 500, jsonBody: { message: 'Server error', error: e } };
    }
